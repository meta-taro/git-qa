...
[English](./README.md) · [配布ページ](https://meta-taro.github.io/git-qa/)
...
1. **データ用の USB ケーブル**で繋ぐ（充電専用だと `adb devices` に出ません）
2. 端末で **設定 → デバイス情報 → ビルド番号を 7 回タップ** → 開発者オプション
3. **USB デバッグ を ON** にし、繋いだときに出る許可ダイアログで **許可**
4. 確認: `adb devices -l` に `device` と出れば OK

> **端末の画面を点けておいてください。**消えていると `screenrecord` は 1 枚も返しません
> （ヘッダの 61 バイトだけが出ます）。その場合はアプリの中央に理由が出ます。
>
> 実機とエミュレータを同時に繋いでいるときは、**アプリの一覧から選べます**。
> 実機の検査を走らせるときは `GIT_QA_ANDROID_SERIAL=<serial>` を付けてください。

**実測（ASUS Zenfone 10 / Android 15 / 1080x2400・2026-09-03）**

| | |
|---|---|
| 実機の検査 7 件 | すべて通過 |
| 端末が名乗る codec | `avc1.64002a`（**High profile / Level 4.2**） |
| エミュレータの codec | `avc1.42c032`（Baseline / Level 5.0） |

**この 2 つが違うので、codec を決め打ちにはできません**（端末の SPS から組み立てています）。
...
> **枠の中に映像が描かれることは、人が見て確認済みです**（2026-09-02・エミュレータ）。
> 実機では操作から adb の出口までの中央値が 154 ms でした（2026-09-02 実測）。

### 検証シートを 1 本走らせる

```bash
pnpm run:sheet sheets/android-settings.tsv
```
...
**１％未満を切り捨てない**（C56）。対応の広さは、この道具が何のためにあるかの説明そのもの。
...
| **Safari** | ✅ **実測**（Version/26.6.1・**WebDriver**・依存なし） |
...
> **はったりを書かない。**この表に「✅ 実測」と書けるのは、**動かして確かめた分だけ**。
> 「動く見込み」と「動いた」を混ぜない。
...
|---|---|
| クリック | タップ |
| 押したままなぞる | スワイプ（速いほどフリック） |
| **同じ所で 0.5 秒以上押す** | **長押し**（メニューを出す操作） |
| **ホイールを回す** | **スクロール**（貯めてから 1 回のなぞりにする。1 回ごとに送ると adb が詰まる） |
| 映像の下の欄に打って Enter | **文字を送る**（**英数字のみ**。端末の入力は IME を通らないため） |
...
**`sheets/android-settings.tsv` は、どの Android にも入っている「設定」アプリを相手にした実物のシート**です。
架空のアプリではないので、端末を繋げばそのまま走ります（**画面が英語表示の端末が前提**。
日本語表示の端末では、手順の鉤括弧の中を書き換えてください）。

シートの見出しに `# 対象:` を書くと、「アプリを起動する」の行き先になります。

```
# 対象: com.android.settings
```

**表示名（「設定」）からは起動できません。**どのパッケージかは端末と地域で変わるので、当てにいくと
別のアプリを起動したまま画面が動き、通ったように見えてしまうためです。

> **AI が 5 件を PASS まで進めることは、実機で 3 回続けて確認しています**（2026-09-04）。
> **まだ無いのは、人が実物を見て判定を置いた `run.json`** です。そこは代われません（C1 / C17）。
...
**相手は Android だけです。**ウェブ・デスクトップは MCP からは触れません
（外部レビュー meta-taro/git-qa#23。README の表が「3 つとも触れる」ように読めていました）。
...
| `device_launch` | アプリを起動する（Android ならパッケージ名で） |
| `device_type` | いま入力先になっている欄へ文字を送る（ASCII のみ） |
