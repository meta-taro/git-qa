...
describe('winArgs', () => {
   ...
   * 欄を指さないので、座標は持たない。**窓だけは要る** —— 前面に出したことを確かめてから送るため。
   */
  it('焦点へ打つ文字は、窓と一緒にそのまま渡す', () => {
    expect(winArgs.keys(1234, '20260903')).toEqual(['keys', '1234', '20260903']);
    expect(winArgs.keys(1, 'git-qa の検証')).toEqual(['keys', '1', 'git-qa の検証']);
  });
