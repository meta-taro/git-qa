...
/**
 * **述語を見ずに、文字探しへ落としていた**（外部レビュー meta-taro/git-qa#27）。
 *
 * > 「押せない」「状態になっている」のように、**画面の文字ではないもの**を
 * > 指して鉤括弧を使う書き方が普通にあります。そこを区別していないため、
 * > **判定として成立しない行が、機械で判定できる行として通ってしまいます。**
 *
 * 実測（報告のとおり再現した）:
 *
 * ```text
 ...
 * ```
 *
 * - 前者は**実装が正しくても必ず落ちる**（画面にその文字は出ない）
 * - 後者は**押せることを一度も確かめずに、「押せない」行へ合格が付く**
 *
 * **鉤括弧の後ろに何が続くか**を見る。**文末ではない** ——
 * 「「…」と表示され、保存されない」のように、
 * **画面の文字を指したうえで別のことも言う**書き方が普通にあるため。
 */
describe('planExpectation — 鉤括弧の後ろの言い方を見る（#27）', () => {
  it('「…と表示される」は、文字探しにする', () => {
    expect(planExpectation('「保存しました」と表示される')).toMatchObject({
      kind: 'contains',
      text: '保存しました',
    });
  });

  it('「…が表示されている」も、文字探しにする', () => {
    expect(planExpectation('「定休日」が表示されている')).toMatchObject({ kind: 'contains' });
  });

  /** **画面に出る文字ではない。**実装が正しくても必ず落ちる判定を作らない。 */
  it('「…」状態になっている は、人へ渡す', () => {
    expect(planExpectation('今日から 2 日後までが「選択できない」状態になっている')).toMatchObject({
      kind: 'hold',
    });
  });

  /** **押せることを確かめていないのに、合格が付いていた。** */
  it('「…」は押せない は、人へ渡す', () => {
    expect(planExpectation('「定休日」の日は押せない')).toMatchObject({ kind: 'hold' });
  });

  /** **「在るか」しか見られない**ので、「出ない」は確かめられない（言っていることの逆になる）。 */
  it('「…と表示されない」は、人へ渡す', () => {
    expect(planExpectation('「エラー」と表示されない')).toMatchObject({ kind: 'hold' });
  });

  /**
   * **続きがあっても、画面の文字を指していれば落とす。**
   * 「と表示され、保存されない」は、前半が画面の文字の話。
   */
  it('画面の文字を指したうえで別のことも言う文は、今までどおり', () => {
    expect(planExpectation('「本文を入力してください」と表示され、保存されない')).toMatchObject({
      kind: 'contains',
      text: '本文を入力してください',
    });
  });
  ...
  /**
   * **但し書きが誘導になっていた**（#27 のいちばん効いている指摘）。
   *
   * > **但し書きに従うほど、判定が間違った方向に確定します。**
   *
   * 「画面で探す文字列が無い」と読んだ人は**鉤括弧を足す。**実際それで通る ——
   * **hold（正直に放棄した状態）から、言っていることの逆を判定する状態へ移る。**
   */
  it('人を間違いへ案内しない（鉤括弧を足せば通る、と読ませない）', () => {
    const zero = planExpectation('配送日カレンダーが表示されない');
    const wrongShape = planExpectation('「定休日」の日は押せない');

    for (const plan of [zero, wrongShape]) {
      expect(plan.kind).toBe('hold');
      // 「鉤括弧を足せ」と読める言い方をしない。
      expect(plan.kind === 'hold' ? plan.reason : '').toContain('画面に出る文字');
    }
    /**
     * **2026-09-24 に、言い分が変わった。**
     *
     * 前は「押せる・選べるは判定できない」と言っていた。
     * **いまは人が読む文を書けるので、`「日本語」が選択できる` は通る。**
     * **通らないのは打ち消しだけ** —— `押せない` が文字探しに化けると、
     * **押せることを確かめずに合格が付く。**
     *
     * **だから、その場で言うのは「打ち消し」のほう。**
     */
    expect(wrongShape.kind === 'hold' ? wrongShape.reason : '').toContain('打ち消し');
  });
});
...
/**
 * **人が読んで分かる期待結果を書けるようにする**（2026-09-24・人の指示）。
 *
 * > 「日本語」と表示される **は、「どこに」が無いです。**
 * > **プルダウンが開閉し、2 番目に「日本語」が選択できる。**
 * > 人がわかる検証シートを作成しなさいと、4、5 回続けて言っています。
 *
 * **鉤括弧の中を機械が見て、前後の文はそのまま人に見せる。**
 * **「どこに」「どうなったか」「何ができるか」は、機械には渡らないが人には渡る。**
 *
 * **否定は今までどおり弾く**（#27）。「「定休日」の日は押せない」が
 * **文字探しに化ける**と、**言っていることの逆**を判定する。
 */
describe('期待結果に、人が読む文を書ける（2026-09-24）', () => {
  const textOf = (said: string): string | undefined => {
    const plan = planExpectation(said);
    return plan.kind === 'contains' ? plan.text : undefined;
  };

  it('人の文のまま書ける。機械は鉤括弧の中だけを見る', () => {
    expect(textOf('プルダウンが開閉し、2 番目に「日本語」が選択できる')).toBe('日本語');
  });

  it('今までの書き方も、そのまま通る', () => {
    expect(textOf('「保存しました」と表示される')).toBe('保存しました');
  });

  it('鉤括弧の前だけに文があってもよい', () => {
    expect(textOf('画面の左上に「接続」と表示される')).toBe('接続');
  });

  /** **否定は弾く。**言っていることの逆を判定させない（#27）。 */
  it('否定は今までどおり弾く', () => {
    expect(textOf('「保存しました」と表示されない')).toBeUndefined();
    expect(textOf('「定休日」の日は押せない')).toBeUndefined();
    expect(textOf('「送信」が押せません')).toBeUndefined();
  });

  it('鉤括弧が 2 つ以上・0 個は、今までどおり人へ渡す', () => {
    expect(textOf('「あ」と「い」が表示される')).toBeUndefined();
