...
| 欄を指して入力する | 押してから `keystroke` | **UI Automation の `SetValue`**（値を置き換える・前面に出ない） |
| 欄を指さずに入力する | 焦点へ `keystroke` | **焦点へ `SendInput` で 1 文字ずつ**（#42・前面に一瞬出る） |
| キー・回す | あり | **あり**（キーは前面に一瞬出る） |
| 録る | ScreenCaptureKit | **無い**（`unsupported`。画面は残る） |
| 相手の入れ替わり検出 | `lsof -d txt` | `QueryFullProcessImageNameW` |

**無いものは「無い」と名乗らせてあります**（録画は `unsupported`）。

**入力の 2 つは、確かめられるものが違います。**欄を指す形（`「欄」に「値」と入力する`）は
**値を丸ごと置き換える**ので、**1 キーごとに走る制限**（日付欄の年を 4 桁で止める、など）を通りません。
それを確かめたいときは、**先に欄を押して焦点を置き、欄を指さずに**書いてください。
...
**欄を指さない形は、そのとき焦点のある所へ打ちます。**焦点を置く行を前に書いてください。
...
- ~~ネイティブの編集領域の本文が読めません~~ → **入力欄の値は `ValuePattern` で読みます**（#14）
- **欄を指さずに打つ形（#42）は、WebView2 の日付欄で実測していません**（2026-09-25 時点。
  こちらの機械は macOS で、ビルドと単体試験までしか通していません）
