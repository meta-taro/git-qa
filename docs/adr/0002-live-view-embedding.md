...
### 実機（エミュレータ）由来の H.264 でも通した

上の測定は ffmpeg で作った代役の映像。**この Mac に Android エミュレータが既にあった**ので、
**scrcpy が実際に吐いた H.264 でも受け手を試した。**

- Pixel 3a / API 31 / arm64 のエミュレータを `-no-window` で起動し、`scrcpy --record` で 60 秒録画
- その mp4 を**再エンコードせず** Annex-B へ詰め替えて（`-c:v copy -bsf:v h264_mp4toannexb`）
  スパイクへ流した。**端末のエンコーダが実際に並べた NAL のまま**受け手に当たる
- スパイクは `LIVE_VIEW_SOURCE` に動画を渡すとそちらを流すようにした

**結果: 復号エラー 0 件。**最初の絵まで 125 ms、復号の待ち 7.5〜7.8 ms、受信＝描画。
**代役の映像でしか試していない、という弱点はここまで潰れた。**

ただし**これはエミュレータであって実機ではない。**そして**確かめたのは復号だけ**で、
端末側のエンコーダ遅延・USB / Wi-Fi の遅延は依然として含まれていない。

**副次の発見: scrcpy は画面が変化したときしかフレームを作る。**60 秒の録画で映像の長さは 33 秒、
平均 10 fps だった（スクロールを止めた時点で止まる）。**証跡の容量は実時間ではなく
画面が変化した量に比例する**ので、人が考えている時間・AI が待っている時間はほぼ容量を食わない。
Issue 007 の試算はこの実測で置き直した。
...
- ~~**実機の scrcpy で測っていない。**~~ → **scrcpy が実際に吐いた H.264 で復号は通した**（エミュレータ・2026-09-02 の追記）。ただし**実機ではない**し、**端末側のエンコーダ遅延・USB / Wi-Fi の遅延は依然として含まれていない**
