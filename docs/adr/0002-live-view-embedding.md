...
### 未解決 — `document.hidden` が常に true

計測中、**`document.hidden` が全ての報告で true** だった（ウィンドウを `alwaysOnTop` + `focus` で
開いても変わらない）。WebKit はページが隠れているとタイマーを間引くため、**約 20 秒ごとに 14 秒前後の
停止**が周期的に出ている。停止を跨いだフレームの遅延は 905 ms まで跳ねる。

上の表の遅延は、**動いている区間の値**であって、この停止を含めていない。

**これが「実際にウィンドウが隠れている」のか「見えているのに hidden と報告されている」のかを
切り分けていない。**後者なら、実使用でライブビューが周期的に固まるという実害になる。
**画面を見ての判断が要る**（product-baseline §29）ので、ここは人の確認待ち。
...
- **macOS では `document.hidden` が常に true で、タイマーが間引かれている。**周期的な停止の原因が「本当に隠れている」のか「見えているのに hidden と報告されている」のか、切り分けていない（追記の末尾）
