...
**Android は動きます**（上の表）。**iPhone は相手にできません。**

**iOS は「未テスト」ではなく「無い」です。**そして**当面やりません**（C74・2026-09-19）。
ロードマップの Phase 6 / `.claude/issues/001`（Windows→iOS）は**見送りに入っています** ——
**この道具はデスクトップアプリの検証から始まっており、スマートフォンはそこに入っていません。**

`pnpm doctor` は**挿したことには気づきます**（macOS のみ・`xcrun devicectl`）。
**気づくだけで、その先はありません。**測る口だけ残してあるのは、
**将来やると決まったときに、一歩目をもう一度作らないため**です。
