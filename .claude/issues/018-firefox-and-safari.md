...
### 2026-09-06（続き）— **Safari も通った**

人が「リモートオートメーションを許可」にチェックを入れた。その場で通した。

```
target:      Safari（Version/26.6.1）
絵:          23,454 バイト
読める文字:  "ローカルで動いているページ / まだ押されていない / 押す"
押した結果:  "ローカルで動いているページ / 押された / 押す"
```

**見る・触る・読むが全部通っている。**「押す」を文字で探して押し、
その結果（「押された」）を読み返している。

**シートを 1 本通すところは、最後の 1 件が人の打鍵待ちで止まった。**
AI が代わりに打鍵を送る道は当てにならない（Issue 016 で分かっている）。
**判定を置くのは人の仕事**なので、そこは人がやる。

### 残っていること

### 案内の場所を間違えていた（2026-09-06・**人の指摘**）

人が「これすでにはいってましたよ」と画面を見せてくれた。**「詳細」タブは済んでいた。**

こちらの案内が**この版の名前と違っていた。**実測で確かめた:

```
Safari のメニューバー: Apple, Safari, ファイル, 編集, 表示, 履歴, ブックマーク, 開発, ウインドウ, ヘルプ
「開発」メニューの中身（末尾）: … キャッシュを空にする / Safari Technology Previewを入手 /
                                デベロッパ設定… / 機能フラグ…
```

**「開発」メニューに「リモートオートメーションを許可」は無い。**
この版では**設定の「デベロッパ」タブ**へ移っている（メニューには「デベロッパ設定…」だけ）。

文言を直した。**版で場所が変わるものは、古い場所も併記する。**
