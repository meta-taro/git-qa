...
4. 宿主から Tauri を起動する所（`?live=<url>` を渡す）。**部品は揃っているので、
   これで枠の中に実際の端末の映像が出る**
...
- **宿主から Tauri を起動する所が無い。**`startLiveSession()` までは出来て検査も通っているが、
  その URL を付けて Tauri を起こす 1 本がまだ無い
