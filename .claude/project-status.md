...
**一本道は繋がった。ただし、代役の端末でしか走らせていない。**
**そして映像が枠の中に描かれるところを、まだ誰も見ていない。**
...
| **枠の中に映像が描かれる** | ❓ **未確認**（画面ロック中はアプリが動かない） |
...
- **手順の語彙が 2 つしかない**（タップ / 入力）。スワイプ・長押し・キー入力は `BLOCKED` になる。
  **足すのは実際のシートで詰まってから**
- **webview が橋に繋いで映像を描くところが未確認。**部品は全部揃い、橋が 6 秒で 2.8 MB
  流すことまで確認したが、**画面がロックされているとアプリが動かない**ため、最後の 1 マスが
  埋まっていない。**人がロックを解いて `pnpm live` を実行する必要がある**
