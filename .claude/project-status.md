...
**端末の映像を画面へ出す道は通した。ただし、映ったところを誰も見ていない。**
...
| | |
|---|---|
| テスト | **268 件・全て通過**（端末を繋ぐと **274 件**） |
| カバレッジ | statements **95.85%** / branch 94.14%（目安 80%） |
| ビルド | `pnpm verify` に含めた（webview 向けの配布物が実際に作れること） |
| 個人情報チェック | `bash .github/scripts/oss-privacy-check.sh` → OK |

**残っている 0% は 3 つ。**`command.ts`（型だけ）/ `main.ts`（配線）/ `webcodecs.ts`
（webview の中でしか動かない・検査していないと明記済み）。
