...
1. **人**: `gh auth login` → push → CI が success することの確認（Issue 003 の完了条件）
2. **人**: 起動した画面を見て、空の 3 カラムが出ていることの確認（§29）
3. Issue 010 — 作る前に既存 OSS を調べる
4. `DESIGN.md` を人が埋める。埋まるまで骨格に色は入れない（§11）

## 技術的決定

`.claude/decisions.md` を参照（C1〜C28）。2026-09-01 に C28（デスクトップ骨格の作り）を追加。
...
- **macOS / Linux で WebCodecs の H.264 復号を未測定**（ADR 0002 の「覆すべき条件」の 1 つ）。Mac で作業できるようになったので、ここは測れる状態になった
