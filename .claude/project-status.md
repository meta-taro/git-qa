...
2. ~~**`DESIGN.md`**~~ — **埋まった**（2026-09-03）。人がアイコン案 B を選び、
   アクセント色 `#0F768E` が決まった。字面（フォント）だけ未定
...
1. **`pnpm app` か `/Applications/git-qa.app` を開き、5 ケースを打鍵で通す** → Issue 004 が閉じる
2. **主指標（C7）を測る対象を選ぶ**（Issue 009）。実案件のどれで測るかは人の判断
3. **版を上げるか決める**（`0.1.0`）。上げると自動更新の下地になる
4. **字面（フォント）を決める**（`DESIGN.md` の残り 1 項目）
...
| | |
|---|---|
| テスト | **550 件・全て通過**（端末を繋ぐと **558 件**）＋ Rust 2 件 |
| カバレッジ | statements **82.34%** / branch 90.88%（目安 80%） |
