...
4. 宿主を 1 本書く。アダプタを繋ぎ、橋を起こし、`?live=<url>` で Tauri を起動する。
   **これで枠の中に実際の端末の映像が出る**
...
| | |
|---|---|
| テスト | **229 件・全て通過**（端末を繋ぐと **235 件**） |
| 内訳 | コア 117 / デスクトップ 43 / 橋 10 / Android アダプタ 59 ＋ 実機 6（既定では飛ぶ） |
...
- **橋を起こして画面へ渡す「宿主」がまだ無い。**アダプタを繋ぎ、橋を起こし、
  `?live=<url>` を付けて Tauri を起動する所が未実装。**部品は全部揃って通しの検査も通っているが、
  それを起動する 1 本が無い**
